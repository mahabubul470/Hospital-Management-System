...
namespace Hospital_Management_System
{
	public partial class Patient : UserControl
	{
		...
		void FillNurseComboBox()
		{
			comboNurse.Items.Clear();
			string sqlString = "select [Name] from [Nurse]";
			DataTable table = DataAcces.ExecuteQuery(sqlString);
			comboNurse.DataSource = table;
			comboNurse.DisplayMember = table.Columns["Name"].ColumnName;
			comboNurse.ValueMember = table.Columns["Name"].ColumnName;
		...
		string getDoc()
		{
			DataRowView dataRow = comboDoctor.SelectedItem as DataRowView;
		...
		void FillDoctorCombo()
		{
			...
			comboDoctor.DataSource = dataTable;
			comboDoctor.DisplayMember = "Name";
			comboDoctor.ValueMember = "Name";
		...
		void fillTextBox()
		{
			...
			diseaseTxtbx.Text = dataTable.Rows[0]["Diseas"].ToString();
